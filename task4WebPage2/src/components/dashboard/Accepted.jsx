...
const Accepted = ({data, index, onComplete, onFail}) => {
  ...
  return (
     <div className='w-[27%] h-80 bg-red-300 shrink-0 rounded-2xl px-4 py-6 flex flex-col gap-6'>
            ...
            <p className='text-xl font-normal'>{description}</p>
             ...
             <div className='flex justify-between'>
                <button 
                    className='text-lg px-1.5 py-1 rounded-xl active:scale-98 bg-green-300'
                    onClick={() => onComplete(index)}
                >
                    Mark as Completed
                </button>
                <button 
                    className='text-lg px-1.5 py-1 rounded-xl active:scale-98 bg-red-500'
